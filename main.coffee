...
Q = (value) -> Qpromise (resolve) -> resolve value
Qpromise = (resolver) ->
  resolved = false
  resolvedValue = null
  resolves = []
...
.then (res) ->
  ...
  getQ 'http://golestan.ut.ac.ir/home/balancer/captcha.aspx', "ASP.NET_SessionId=#{sid}", 'binary'  
  .then (res) ->
    writeFileQ 'Captcha.gif', res.body, 'binary'
...
.then (data) ->
  ...
  .then -> getQ 'https://golestan.ut.ac.ir/Forms/AuthenticateUser/AuthUser.aspx?fid=0;1&tck=&&&lastm=20141001150602', "ASP.NET_SessionId=#{data.sid}"
  .then (res) ->
    eventValidation = /id="__EVENTVALIDATION".*/g.exec(res.body)[0]
    eventValidation = eventValidation.substr 30
    eventValidation = eventValidation.substr 0, eventValidation.length - 4
    ...
    postData =
      ...
      Frm_Type: ''
      TicketTextBox: ''
      TxtMiddle: '<r F51851="" F80351="810190498" F80401="123456" F83181="" F51701=""/>'
    postQ 'https://golestan.ut.ac.ir/Forms/AuthenticateUser/AuthUser.aspx?fid=0%3b1&tck=&&&lastm=20141001150602', postData, "ASP.NET_SessionId=#{data.sid}; u=; su=; ft=; f=; lt=; seq="
    .then (res) ->
...
.then -> console.log 'Done. Check "Results.txt"'
